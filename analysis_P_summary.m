clear; close all;

file_list = {'miura-ori','recweave','huffman_waterbomb','huffman_waterbomb','langoval','HexTri','lang_honey','honey_comb','auxetic_triangle','triPerfTess' ...
            };

k_all = [1,2,4,8,16,32];       % Number of sampled constraints per step
rule_all = [1,2];              % Selection rules
n_sim = 100;                   % Number of simulations per setup
cmap = color_scheme(length(k_all));  % Color map for k values

for i = 1:numel(file_list)
    file = file_list{i};  % Extract file name

    fprintf('Processing file: %s\n', file);

    % Assign plot name
    if strcmp(file, 'recweave')
        plot_name = 'H. Rect Weave';
    elseif strcmp(file, 'auxetic_triangle')
        plot_name = 'Auxetic Tri';
    elseif strcmp(file, 'miura-ori')
        plot_name = 'Miura-Ori';
    elseif strcmp(file, 'HexTri')
        plot_name = 'Hex/Tri';
    elseif strcmp(file, 'honey_comb')
        plot_name = 'Kiri. Honeycomb';
    elseif strcmp(file, 'huffman_waterbomb')
        plot_name = 'H. Waterbombs';
    elseif strcmp(file, 'lang_honey')
        plot_name = 'Lang Honeycomb';
    elseif strcmp(file, 'langoval')
        plot_name = 'Lang Oval';
    elseif strcmp(file, 'leafy')
        plot_name = 'Leafy';
    elseif strcmp(file, 'triPerfTess')
        plot_name = 'Perforated Tri';
    else
        plot_name = 'Unknown Pattern';
    end

    % Assign L values
    if strcmp(file, 'recweave') || strcmp(file, 'huffman_waterbomb')
        L_all = [22, 33, 44];
    elseif strcmp(file, 'sim') || strcmp(file, 'leafy')
        L_all = [10,15,20];
    else
        L_all = [2, 3, 4];
    end


    for rule = rule_all
        for L_size = L_all
            for idx = 1:length(k_all)
                k = k_all(idx);

                % === Load preprocess mesh info ===
                preprocess_file = sprintf('pre_process/origami_data/%s/%s_%d_25%%_data.mat', ...
                                          file, file, L_size);

                if ~exist(preprocess_file, 'file')
                    warning('Missing preprocess file: %s', preprocess_file);
                    continue;
                end

                load(preprocess_file);  % Expects: F, E, L, facet_num, edge_num
                edge_labels = L;  % Rename L (edge label vector) to avoid conflict
                facet_num = size(F, 1);

                % === Build adjacency graph ===
                facet_groups = num2cell(1:facet_num);
                edge_to_facet = containers.Map;

                for fidx = 1:facet_num
                    f = F(fidx, :);
                    edges = sortrows([f([1 2]); f([2 3]); f([3 1])], 2);
                    for j = 1:3
                        e = sort(edges(j, :));
                        key = sprintf('%d_%d', e(1), e(2));
                        if isKey(edge_to_facet, key)
                            edge_to_facet(key) = [edge_to_facet(key), fidx];
                        else
                            edge_to_facet(key) = fidx;
                        end
                    end
                end

                edge_num = size(E,1);

                for eidx = 1:edge_num
                    if edge_labels(eidx) == 1  % planar diagonal
                        e = sort(E(eidx, :));
                        key = sprintf('%d_%d', e(1), e(2));
                        if isKey(edge_to_facet, key)
                            adj = edge_to_facet(key);
                            if numel(adj) == 2
                                f1 = adj(1); f2 = adj(2);
                                g1 = find(cellfun(@(g) ismember(f1, g), facet_groups));
                                g2 = find(cellfun(@(g) ismember(f2, g), facet_groups));
                                if g1 ~= g2
                                    facet_groups{g1} = unique([facet_groups{g1}, facet_groups{g2}]);
                                    facet_groups(g2) = [];
                                end
                            end
                        end
                    end
                end

                % === Compute tri_ratio ===
                tri_ratio = sum(cellfun(@numel, facet_groups) == 1) / numel(facet_groups);
              

                % === Load simulation DoF data ===
                sim_file = sprintf('simulation_results/%s/%s_%d_25%%_data/%s_%d_25%%_data_k_%d_rule_%d_n_100.mat', ...
                                   file, file, L_size, file, L_size, k, rule);

                if ~exist(sim_file, 'file')
                    warning('Simulation file not found: %s', sim_file);
                    continue;
                end

                load(sim_file);  % loads dof_all
                [n_sim, max_steps] = size(dof_all);
                r = linspace(0, 1, max_steps);
                dof_all = cummin(dof_all, 2);  % Force non-increasing
                min_dof = max(dof_all(:,end));
                P = sum(dof_all <= min_dof, 1) / n_sim;

                total_facet=max_steps-1;

                % === Save output ===
                save_folder = 'P_summary_data';
                if ~exist(save_folder, 'dir'), mkdir(save_folder); end

                save_filename = sprintf('%s/P_%s_L%d_k%d_rule%d.mat', ...
                                        save_folder, file, L_size, k, rule);
                save(save_filename, 'P', 'k', 'L_size', 'rule', 'total_facet', 'r', 'tri_ratio');
            end
        end
    end
end


%%

load('P_summary_data/P_auxetic_triangle_L4_k1_rule1.mat');

% %% Plot k vs critical r for each L and rule
% r_star_all = zeros(length(L_all), length(k_all));
% r_0_all    = zeros(length(L_all), length(k_all));
% r_1_all    = zeros(length(L_all), length(k_all));
% r_01_all   = zeros(length(L_all), length(k_all));
% r_09_all   = zeros(length(L_all), length(k_all));
% r_025_all  = zeros(length(L_all), length(k_all));
% r_075_all  = zeros(length(L_all), length(k_all));
% 
% for rule = [2, 1]  % Plot rule 2 first
%     for L = L_all
%         id_L = find(L_all == L);
% 
%         for idx = 1:length(k_all)
%             k = k_all(idx);
%             id_k = find(k_all == k);
% 
%             filename = sprintf('simulation_results_office/%s/%s_%d_25%%_data/%s_%d_25%%_data_k_%d_rule_%d_n_100.mat', ...
%                                file, file, L, file, L, k, rule);
% 
%             if exist(filename, 'file')              
%                 load(filename);  % loads dof_all
%                 [n_sim, max_steps] = size(dof_all);
%                 r = linspace(0,1,max_steps);
% 
%                 min_dof = min(dof_all(:));
%                 P = sum(dof_all == min_dof, 1) / n_sim;
% 
%                 % Record critical rho thresholds
%                 idx_50 = find(P >= 0.5, 1);
%                 if ~isempty(idx_50), r_star_all(id_L, id_k) = r(idx_50); end
% 
%                 idx_0 = find(P == 0, 1, 'last');
%                 if ~isempty(idx_0), r_0_all(id_L, id_k) = r(idx_0); end
% 
%                 idx_1 = find(P == 1, 1);
%                 if ~isempty(idx_1), r_1_all(id_L, id_k) = r(idx_1); end
% 
%                 idx_01 = find(P <= 0.1, 1, 'last');
%                 if ~isempty(idx_01), r_01_all(id_L, id_k) = r(idx_01); end
% 
%                 idx_09 = find(P >= 0.9, 1);
%                 if ~isempty(idx_09), r_09_all(id_L, id_k) = r(idx_09); end
% 
%                 idx_025 = find(P <= 0.25, 1, 'last');
%                 if ~isempty(idx_025), r_025_all(id_L, id_k) = r(idx_025); end
% 
%                 idx_075 = find(P >= 0.75, 1);
%                 if ~isempty(idx_075), r_075_all(id_L, id_k) = r(idx_075); end
%             else
%                 warning('File not found: %s', filename);
%             end
%         end
% 
%         % Plot k vs rho*
%         figure;
%         hold on;
%         if rule == 1
%             plot([0, 34], [(4*L - 4)/L^2, (4*L - 4)/L^2], 'k--', 'LineWidth', 1);
%         end
% 
%         plot(k_all, r_star_all(id_L,:), '-o', ...
%              'Color', [201 0 22]/255, 'MarkerFaceColor', [201 0 22]/255, ...
%              'LineWidth', 2);
% 
%         xlabel('k');
%         ylabel('$\rho^*$', 'Interpreter', 'latex');
%         title(['L = ', num2str(L), ', Rule ', num2str(rule)]);
%         set(gca, 'FontSize', 18, 'LineWidth', 2);
%         xlim([0 34]); ylim([0 1]);
%     end
% end
% 
% 
% 



